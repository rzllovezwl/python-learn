...
- 通过对象对类中成员重新赋值或通过对象添加成员时，对应成员回保存在对象中，而不会修改类成员
...
- 判别对象的位置
    ...
    - 对象外部
    - 子类中
- [python中下划线的使用](https://blog.csdn.net/haiyang_duan/article/details/79095213)
- 私有
    ...
    - 在成员前面添加两个下划线即可
            ...
            class person():
                ...
                # __age就是私有成员
                __age = 18
   - python的私有不是真私有，是一种name mangling的改名策略，可以使用对象._classname_attributename访问
- 受保护的封装 protected
...
- 菱形继承/钻石继承问题
    ...
    - 关于多继承的MRO
        ...
        - MRO列表的计算原则：
            ...
            - 如果多个类继承了同一个父类，子类中只会选取继承语法括号中第一个父类的父类

## 3.3 多态
- 多态就是同一个对象在不同情况下有不同的状态出现
- 多态不是语法，是一种设计思想
- 多态性：一种调用方式，不同的执行效果
- 多态：同一事物的多种形态，h2o有液态、固态、气态
- [多态和多态性](https://www.cnblogs.com/luchuangao/p/6739557.html)
- Mixin设计模式（利用多继承实现）
    - 主要采用多继承方式对类的功能进行扩展
    - [Mixin](https://www.zhihu.com/question/20778853) 
- 我们使用多继承语法来实现Mixin
- 规则：
    - 首先他必须表示某一单一功能，而不是某个物品
    - 职责必须单一，腐国有多个功能，则写多个Minxin
    - Mixin不能依赖子类的实现
...
- 无论哪种修饰符都是为了对成员属性进行相应的控制
    ...
    - property：适用于当前类中适用，可以控制一个类中多个属性
    - 属性修饰符：适用于当前类中使用，控制一个类中的一个属性
# 6.类的内置属性


            __dift__:以字典的方式显示类的成员组成
            __doc__:获取类的文档信息
